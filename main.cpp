...
//        sleep(1);
    ...
    while(getline(plik,linia))
    {
        ...
        getline(sstr,temp,'|');
        wczytanyRekord.id=atoi(temp.c_str());
        getline(sstr,temp,'|');
        wczytanyRekord.idUzytkownika=atoi(temp.c_str());
...
void nadpiszPlik(vector<wpis> *ksiazkaAdresowa,int indeks){
    fstream plik,plik2;
    string temp,nazwaOryg{"kontakty.txt"},nazwaTymcz{"kontaktytemp.txt"};
    plik.open(nazwaOryg,ios::in);
    plik2.open(nazwaTymcz,ios::out);
    if(plik.good()==false)
    {
        cout<<"Nie udalo sie zaimportowac kontaktow"<<endl;
        Sleep(1000);
    ...
    while(getline(plik,linia))
    {
        ...
        getline(sstr,temp,'|');
        getline(sstr,temp,'|');
        if (atoi(temp.c_str())!=indeks)
            plik2<<linia<<endl;
    }
    plik2.close();
    plik.close();
    zapisDoPliku(ksiazkaAdresowa,nazwaTymcz);
